...
// File: contracts/KeyVault.sol
contract KeyVault is Ownable, Pausable {
    ...
    struct UserInfo {
        ...
        uint256 keyAtLastUserAction; // keeps track of key deposited at the last user action
        uint256 lastUserActionTime; // keeps track of the last user action time
    }

    IERC20 public immutable token; // Key token
    ...
    /**
     * @notice Constructor
     * @param _token: Key token contract
    ...
    /**
     * @notice Deposits funds into the Key Vault
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in KEY)
     */
    function deposit(uint256 _amount) external whenNotPaused notContract {
        ...
        user.keyAtLastUserAction = user.shares.mul(balanceOf()).div(
            totalShares
        );
    ...
    /**
     * @notice Reinvests KEY tokens into MasterChef
     * @dev Only possible when contract not paused.
     */
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the Key Vault
     */
     ...
     * @return Expected reward to collect in KEY
     */
    function calculateHarvestRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingReward(
            0,
            address(this)
        );
     ...
     * @return Returns total pending KEY rewards
     */
    ...
    /**
     * @notice Withdraws from funds from the Key Vault
